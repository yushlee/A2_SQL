-- 1369.Get the Second Most Recent Activity 獲取第二個最近的活動
...
--  Alice 最近的活動是從 2020-02-24 到 2020-02-28 的旅行
--  之前她從 2020-02-21 到 2020-02-23 跳舞。
--  Bob只有一個記錄，只取一個。
...
WITH T AS (
    SELECT USERNAME, ACTIVITY, STARTDATE, ENDDATE,      
      RANK() OVER (PARTITION BY USERNAME ORDER BY STARTDATE DESC) USER_START_RANK,      
      COUNT(USERNAME) OVER (PARTITION BY USERNAME) CNT
    FROM USERACTIVITY
)
SELECT USERNAME, ACTIVITY, STARTDATE, ENDDATE
FROM T
WHERE USER_START_RANK = 2 OR CNT = 1;
